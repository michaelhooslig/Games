...
public class Troll
{
  ...
  private int monDmg = 4;
	private double monAttack = .35;
  private double monDefence = .3;
  ...
  public Troll(int hardness)
	{
		difficulty = hardness;
		monHealth = ((int)(Math.random()*6)+14) * difficulty;
	}
  ...
  public int monLoot()
  {
	...
	{
		return 15 * (int)(difficulty * levelBuff *1.5);
	}
	...
	public int trollCombat(int Php, int PmeleeDmg, int PrangeDmg, double PmeleeAtt, double PrangeAtt, double dex, double back, double man, int throw1, int spark, int bolt, int fire, int ice, int quake, double cspell, double rspell)
	{
		health = Php;
		melee = PmeleeDmg;
		range = PrangeDmg;
		...
		if (true)
    {
      ...
      while (monHealth > 0)
      {
        System.out.println("\n Cave Troll's stats: Troll has " + monHealth + " Hit Points left and deals " + monDmg + " every attack.\n");
  ...
  {
    return "Troll has " + monHealth + " Hit Points left and deals " + monDmg + " every attack.";
  }
